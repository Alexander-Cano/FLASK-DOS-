...
R: Que con las variables de python es más fácil ya que en html solo tienes que llamar a la variable, no escribir una
etiqueta para cada texto
2. Si el profesor cambia mañana, ¿cuantos archivos necesitas editar con este enfoque?
R: Solo la variable de python
3. ¿Que ventaja tiene usar `{{ }}` en lugar de escribir texto fijo?
R: Usando las llaves se llama a la variable de python que cambia dependiendo del app.py
4. ¿Donde se ejecuta Jinja2: en la computadora del usuario o en el servidor?
R: En la computadora del usuario
