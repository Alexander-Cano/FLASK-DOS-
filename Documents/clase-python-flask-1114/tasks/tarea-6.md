...
R: Imagino que es una libreria que al usarla crea tu propia base de datos dentro de la carpeta seleccioanda
2. ¿Que diferencia hay entre `db.session.add()` y `db.session.commit()`?
R: En una añades información extra y el commit es para hacer cambios
3. ¿Por que `unique=True` es importante en el email?
R: Porque con este se evita repetir el email
