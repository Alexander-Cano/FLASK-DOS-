...
R: Porque es más fácil que alguien las vea
2. ¿Que diferencia hay entre crear un Usuario y loguear?
R: Creando un usuario se almacena en una base de datos, al loggear entras a un usuario ya creado
3. ¿Como protegeria la ruta `/estudiantes` para que solo profesor pueda verla?
R: Con el código que deja entrar nada más al profesor 
" if 'usuario_id' not in session or session['rol'] != 'profesor':
        return redirect(url_for("login"))"
